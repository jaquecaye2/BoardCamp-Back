...
export async function listarAlugueis(request, response) {
  ...
  if (customerId) {
    ...
    const { rows: alugueis } = await connection.query(`SELECT * FROM rentals WHERE "customerId"=${customerId}`);
    ...
    // buscar todos os alugueis no BD e enviá-los para o front
    const { rows: alugueis } = await connection.query(`SELECT * FROM rentals WHERE "gameId"=${gameId}`);
